...
show_help() {
    ...
    echo " Basic Commands:"
    echo "  i       Enter insert mode (insert before current line, enter '.' to end)"
    ...
    echo "  g line# Go to specific line"
...
load_file() {
    ...
    if $modified; then
        read -p "Current content not saved, confirm load new file? (y/n) " confirm
        [[ "$confirm" != "y" ]] && return
    fi
    mapfile -t file_content < "$1"
    current_line=0
    echo "Loaded file: $1 (${#file_content[@]} lines total)"
    filename="$1"
    modified=false
}
...
next_match() {
    if (( ${#search_results[@]} == 0 )); then
        echo "?Error: Please perform search first"
        return
    fi
    ((search_index++))
    if (( search_index >= ${#search_results[@]} )); then
        search_index=0
        echo "?Returned to first match"
    fi
    current_line="${search_results[$search_index]}"
    print_line
}
...
confirm_exit() {
    if $modified; then
        read -p "You have unsaved changes, confirm quit? (y/n) " confirm
        [[ "$confirm" != "y" ]] && return 1
    fi
    return 0
}
